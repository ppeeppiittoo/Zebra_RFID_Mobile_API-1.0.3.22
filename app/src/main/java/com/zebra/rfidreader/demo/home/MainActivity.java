...
import com.zebra.rfidreader.demo.common.ResponseHandlerInterfaces.BluetoothDeviceFoundHandler;
...
import com.zebra.rfidreader.demo.settings.SettingsContent;
...
public class MainActivity extends ActionBarActivity implements Readers.RFIDReaderEventHandler {
    ...
    private static final String MSG_READ = "Leyendo Etiquetas";
    private static final String MSG_WRITE = "Escribiendo Datos";
    private static final String MSG_LOCK = "Ejecutando Comando De Bloqueo";
        ...
        if (isBluetoothEnabled()) {
            if (Application.mConnectedReader != null && Application.mConnectedReader.isConnected()) {
                ...
                if (!Application.mIsInventoryRunning) {
                    ...
                    if (fragment != null && fragment instanceof InventoryFragment && !((InventoryFragment) fragment).getMemoryBankID().equalsIgnoreCase("none")) {
...
//                                    Application.isLocatingTag = true;
    ...
    /*
    * metodo para ligar boton de lista de lectores con la lista de lectores en el menu de inicio
    * */
    public void rListClicked (View view){selectItem(7);}
    ...
    //CAMBIAMOS EL NUMERO AL SELECCIONAR EL ITEM DE ABOUT AL DE AJUSTES PARA PONERLO EN LA PANTALLA DE INICIO
    public void aboutClicked() {
        selectItem(4);
    } //este evento de click era para mostrar el about, pero le cambiamos el item para que mostrar la vista de ajustes
    ...
    private void readerDisconnected(ReaderDevice readerDevice) {
        ...
        if (Application.NOTIFY_READER_CONNECTION)
            sendNotification(Constants.ACTION_READER_DISCONNECTED, "Desconectado de " + readerDevice.getName());
        clearSettings();
...
//            ((AccessOperationsFragment) fragment).deviceConnected(device);
        ...
        if (Application.NOTIFY_READER_CONNECTION)
            sendNotification(Constants.ACTION_READER_CONNECTED, "Conectado a " + device.getName());
    }
    ...
    public void RFIDReaderAppeared(ReaderDevice device) {
        ...
        if (Application.NOTIFY_READER_AVAILABLE) {
            if (!device.getName().equalsIgnoreCase("null"))
                sendNotification(Constants.ACTION_READER_AVAILABLE, device.getName() + " está disponible.");
        }
    }
    ...
    public void RFIDReaderDisappeared(ReaderDevice device) {
        ...
        if (Application.NOTIFY_READER_AVAILABLE)
            sendNotification(Constants.ACTION_READER_AVAILABLE, device.getName() + " no está disponible.");
    }
        ...
        protected void onPostExecute(Boolean result) {
            if (!isCancelled()) {
                if (result)
                    ...
                    sendNotification(Constants.ACTION_READER_CONN_FAILED, "Falló la conexión!! Fue recibido");
                    try {
                        readerDevice.getRFIDReader().disconnect();
                    } catch (InvalidUsageException e) {
                        e.printStackTrace();
